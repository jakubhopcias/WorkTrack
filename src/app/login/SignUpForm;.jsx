...
export default function SignUpForm({ onSwitch }) {
  ...
  return (
    <form className="login-form" onSubmit={hadnleSignUp}>
      <input onChange={e=> setEmail(e.target.value)} name="email" type="email" placeholder="Twój email" />
      <input onChange={e=> setPassword(e.target.value)} name="password" type="password" placeholder="Hasło" />
      {error && (<p>{error}</p>)}
      {success && (<p>{success}</p>)}
